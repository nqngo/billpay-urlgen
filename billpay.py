...
@click.option(
    "-f",
    "--datafile",
    help="The Yaml file that store payment profile.",
...
def cli(datafile, amount, biller, reference, paymentmethod, email):
    ...
    with open(datafile, "r+") as f:
        ...
        data["reference"] = (
            data["reference"] if reference is None else reference.replace(" ", "")
        )
        ...
        if amount is None and biller is None and reference is None and email is None:
            data["amount"] = click.prompt(
                "Payment amount", type=click.FLOAT, default=data["amount"]
            )
            data["biller"] = click.prompt(
                "Billpay code", type=click.INT, default=data["biller"]
            )
            data["reference"] = click.prompt(
                "Reference no", default=data["reference"]
            ).replace(" ", "")
            data["paymentmethod"] = click.prompt(
                "Payment method", default=data["paymentmethod"]
            )
